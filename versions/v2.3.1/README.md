# Kit v2.3.0 — có gì mới

**Nguyên tắc của bản này: ĐỌC thì tự do, GHI thì chỉ khi đã thấy thiếu.**

Cả ba lần mất dữ liệu của dự án đều do một lệnh GHI chạy lúc không cần —
12/08 `repair --apply` (555 mục rác), 18/08 `clean-junk --apply` (dọn mất mục thật),
26/08 `fix-stale --only 1,2,3,4` (REDANCE-1715 tụt 89 → 48 đoạn nội dung).
Chưa lần nào mất vì QUÊN chạy script.

| Thay đổi | Vì sao |
|---|---|
| `safe-quit.sh` KHÔNG còn chạy `repair --apply` | Nó chạy mỗi lần thoát, kể cả khi không thiếu gì — chính là thứ đẻ ra 555 mục rác. Nay: thoát êm → snapshot → kiểm tra chỉ đọc → **in ra lệnh** nếu thiếu. |
| `--only` nhận **mã phiên 8 ký tự**, không nhận số thứ tự | Danh sách đánh số lại sau mỗi lần chạy → cùng một lệnh trỏ vào hội thoại khác nhau giữa hai lần. Mã phiên bất biến. |
| **`repair`: CHỐT SỐ 2 — một hội thoại một mục** | `repair --apply` vẫn đang đẻ mục trùng: 23/08 tạo 14 mục "Phong thủy" trong một giây, 18/08 tạo 6 mục "REDANCE-1715". Nhánh `distinct=True` vô hiệu hoá bộ lọc rẽ nhánh cũ. Nay gom theo (dự án, tiêu đề), chỉ tạo 1. Kiểm chứng: 21 → 3. |
| **`dedup-entries.sh`** (mới) | Dọn 35 mục trùng đang tồn tại. Giữ bản có tin nhắn MỚI NHẤT, in rõ % trùng và số đoạn cũ sẽ bị ẩn. |
| **`restore-lost-entries.sh`** (mới) | Bịt điểm mù: repair bỏ qua MỌI đoạn nén nên hội thoại mà mọi đoạn đều là đoạn nén không bao giờ được tạo mục. Đo 26/08: 918 đoạn bị bỏ qua, **7 hội thoại mất hẳn khỏi danh sách**. Chỉ tạo mục khi cả nhóm không có mục nào → không thể đẻ trùng. |
| `--fix-stale` trỏ sang bản **đủ nhất**, không phải bản **mới nhất** | Đoạn mới nhất của hội thoại dài thường vừa bị nén → rỗng ruột. REDANCE-1715: mới nhất 48 đoạn nội dung, đoạn trước đó 267. |
| `safe-quit.sh` mặc định KHÔNG mở lại app | Tên script nói "quit". Muốn mở lại thì `--reopen`. |
| `clean-junk-entries.sh` không dọn mục mới nhất của mỗi tiêu đề | Tiêu chí nhận diện rác không phân biệt được mục thật. Nay dù nhận sai, mỗi tiêu đề luôn còn ít nhất một mục. |

**Cần biết:** không file đơn lẻ nào chứa cả một hội thoại dài. REDANCE-1715 có 699 đoạn,
hợp lại 13.183 đoạn nội dung; đoạn giàu nhất chỉ chứa 2%. Mọi thao tác `--fix-stale` là
ĐÁNH ĐỔI, không phải khôi phục — muốn mở ra thấy đủ thì phải hợp nhất (`merge-branches.sh`).

**Gõ `versions/latest/bin/…`, đừng gõ số phiên bản.** v2.2.0 vẫn còn nguyên để tra cứu,
nhưng nó chứa đúng hai thứ đã gây mất dữ liệu ngày 26/08.
