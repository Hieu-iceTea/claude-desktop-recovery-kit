...
import json, glob, os, shutil, hashlib, collections, datetime, re
    ...
    for line in open(p, errors="replace"):
        ...
        if isinstance(cc,str):
            ...
            for q in cc:
                ...
                ty=q.get("type")
                if ty=="text":
                    if q.get("text","").strip(): parts.append(q["text"])
                elif ty=="tool_use":
                    tool.append("TU:"+json.dumps(q.get("input"),sort_keys=True,ensure_ascii=False))
                elif ty=="tool_result":
                    v=q.get("content")
                    tool.append("TR:"+(v if isinstance(v,str) else json.dumps(v,sort_keys=True,ensure_ascii=False)))
                elif ty=="thinking":
                    if q.get("thinking","").strip(): tool.append("TH:"+q["thinking"])
        def _add(dst,ps):
            if not ps: return
        ...
        _add(fp,parts)          # fp  = CHỮ  → dùng để NHẬN DẠNG (union-find)
        _add(fpx,parts+tool)    # fpx = ĐẦY ĐỦ → dùng để đo PHỦ KÍN
    return dict(fp=fp, fpx=fpx, n=n, last=last)
...
for f in glob.glob(os.path.join(IDX,"local_*.json")):
    ...
    proj=normproj(os.path.basename(os.path.dirname(tpath[sid]))) if sid in tpath else "(không rõ)"
...
for (proj,title),mem in groups.items():
    ...
    def find(x):
        ...
        return x
    # Nối hai mục khi chúng dùng chung ÍT NHẤT 2 đoạn nội dung.
    # Ngưỡng 1 quá lỏng: một câu mở đầu lặp lại (prompt mẫu, lời chào) đủ để nối
    # hai hội thoại rời nhau. Gặp thật 26/08/2026: ff4ee253 và 9d10be11 chỉ chung
    # 1/17 đoạn (6%) mà vẫn bị gom, khiến một hội thoại riêng bị gắn nhãn
    # "⤷ lưu trữ" của hội thoại khác. repair-missing-sessions đã dùng ngưỡng 2
    # từ trước — hai script phải cùng một thước.
    ...
    for _root,cl in clusters.items():
        ...
        # PHỦ KÍN đo bằng vân tay ĐẦY ĐỦ (fpx: chữ + tool_use + tool_result +
        # thinking). Vân tay CHỮ (fp) chỉ dùng để NHẬN DẠNG ở union-find bên trên.
        # Tách hai vai trò vì chúng đòi hỏi ngược nhau: nhận dạng cần vân tay
        # ĐẶC TRƯNG (vân tay tool bị dùng chung tới 12 hội thoại ⇒ nối nhầm),
        # còn phủ kín cần vân tay ĐẦY ĐỦ (thiếu tool ⇒ dọn nhầm mục còn nội dung).
        universe=set().union(*[info[f]["fpx"] for f,_,_ in cl])
        primary=max(cl, key=lambda x:(info[x[0]]["last"] or "", len(info[x[0]]["fpx"])))
        if NEWEST:
            keep=[primary]
        else:
            # PHỦ KÍN: mục mới nhất trước, rồi tham lam thêm mục nào bù được
            # nhiều nội dung chưa phủ nhất, cho tới khi phủ hết universe.
            keep=[primary]; cov=set(info[primary[0]]["fpx"])
            rest=[x for x in cl if x is not primary]
            while cov!=universe and rest:
                nxt=max(rest, key=lambda x: len(info[x[0]]["fpx"]-cov))
                if not (info[nxt[0]]["fpx"]-cov): break
                keep.append(nxt); cov|=info[nxt[0]]["fpx"]; rest.remove(nxt)
        kept_fp=set().union(*[info[f]["fpx"] for f,_,_ in keep])
