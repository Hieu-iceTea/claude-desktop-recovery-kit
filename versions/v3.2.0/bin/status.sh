...
if [ -d "$SHARED" ]; then
  echo "  ✅ tồn tại — $(find "$SHARED" -maxdepth 1 -name 'local_*.json' 2>/dev/null | wc -l | tr -d ' ') phiên"
else
...
while IFS= read -r d; do
  ...
  if [ -L "$d" ]; then
    ...
    n=$(find "$d" -maxdepth 1 -name 'local_*.json' 2>/dev/null | wc -l | tr -d ' ')
...
if [ -d "$SNAPDIR" ] && [ -e "$SNAPDIR/latest" ]; then
  ...
  NSNAP="$(find "$SNAPDIR" -maxdepth 1 -type d -name 'snap-*' 2>/dev/null | wc -l | tr -d ' ')"
  if [ "$AGE_H" -le 48 ]; then
