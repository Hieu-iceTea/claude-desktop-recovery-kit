#!/usr/bin/env bash
# =============================================================================
# clean-orphan-transcripts.sh   (v2.0.0)
...
#   • --apply : CHUYỂN vào thùng rác recovery-data/orphan-trash-<TS>/ (hoàn tác được), hỏi xác nhận.
...
TS=$(date +%Y%m%d-%H%M%S)
if [ "$MODE" = "apply" ]; then
  TRASH="$KIT/recovery-data/orphan-trash-$TS"
