#!/usr/bin/env bash
# =============================================================================
# repair-missing-sessions.sh   (v2.2.0)
#
...
#     repair-missing-sessions.sh --fix-stale --apply --only 1,4 # chỉ sửa mục 1 và 4
# =============================================================================
set -uo pipefail
...
import json, glob, os, sys, time, uuid, itertools, datetime, shutil, hashlib
...
ONLY   = [int(x) for x in os.environ.get("ONLY", "").replace(" ", "").split(",") if x.isdigit()]
...
def read_meta(path):
        ...
        for line in fh:
            ...
            if t in ("assistant", "user"):
                ...
                if isinstance(cc, str):
                    ...
                    for _p in cc:
                        if isinstance(_p, dict) and _p.get("type") == "text" and _p.get("text", "").strip():
                            _parts.append(_p["text"])
                if _parts:
                    _flat = " ".join(" ".join(_parts).split())[:300]
                    if len(_flat) >= 10 and not _flat.startswith(
                            ("This session is being continued", "Caveat:", "[Request interrupted")):
                        fp_all.add(hashlib.sha1(_flat.encode()).hexdigest())
            if t == "assistant":
    ...
    return {
        ...
        "fp": fp, "fp_all": fp_all, "is_cont": bool(is_cont),
    }
...
# ---------- --fix-stale: mục Recents đang trỏ vào bản rẽ nhánh CŨ ----------
if FIXSTALE:
    stale = []
    for key, members in chains.items():
        ...
        if not hit or len(ids) < 2 or ids[-1] in indexed:
            continue                      # không có mục, hoặc đã trỏ bản mới nhất
        cur = hit[-1]
    ...
    print(f"🔁 fix-stale v{VER} — mục Recents đang mở BẢN CŨ")
    print(f"   {'🟢 THỰC THI (--apply)' if APPLY else '🔍 XEM TRƯỚC — chưa ghi gì'}\n")
    if not stale:
        print("✅ Không mục nào bị tụt. Mọi hội thoại đều mở đúng bản mới nhất.")
        sys.exit(0)
    ...
    for i, (title, idxfile, cm, nm) in enumerate(stale, 1):
        ...
        delta = gain - lose
        flag = "🔴" if lose else ("🟢" if gain else "⚪")
        mark = " " if i in sel else "·"
        print(f"[{i}]{mark}{flag} {title}")
        print(f"     đang mở : {cm['cli'][:8]}  {len(cur_fp):>4} tin nhắn  → {when(cm)}")
        print(f"     bản mới : {nm['cli'][:8]}  {len(new_fp):>4} tin nhắn  → {when(nm)}"
              f"   (+{gain} / -{lose})")
        if lose and i not in (ONLY or []):
            print(f"     ⚠️  ĐỔI SẼ MẤT {lose} tin nhắn — mặc định BỎ QUA.")
            print(f"         Vẫn muốn đổi thì nêu đích danh: --only {i}\n")
            continue
        if i not in sel:
    ...
    if APPLY and changed:
        ...
        print("Chọn riêng vài mục:  --fix-stale --apply --only 1,2,4")
    sys.exit(0)
