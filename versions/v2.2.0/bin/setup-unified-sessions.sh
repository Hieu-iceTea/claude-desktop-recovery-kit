#!/usr/bin/env bash
# =============================================================================
# setup-unified-sessions.sh   (claude-desktop-recovery-kit v2.1.0)
# -----------------------------------------------------------------------------
...
while IFS= read -r d; do
  [ -n "$d" ] || continue
  n=$( { ls "$d"/local_*.json 2>/dev/null || true; } | wc -l | tr -d ' ')
...
TS=$(date +%Y%m%d-%H%M%S); BK="$RECOVERY/$TS-apply-v$VER"; mkdir -p "$BK/orig-folders"
...
# (v2.1.0) Ảnh chụp trạng thái NGAY TRƯỚC khi đổi — để đối chiếu before/after & rollback an toàn
"$(dirname "${BASH_SOURCE[0]}")/status.sh" > "$BK/status-before.txt" 2>&1 || true
# (v2.1.0) MANIFEST: ghi version + lệnh rollback chính xác theo đúng version
{ echo "MANIFEST — claude-desktop-recovery-kit backup"
  echo "Loại backup   : setup --apply (ROLLBACK được)"
  echo "Phiên bản kit : v$VER"
  echo "Script tạo    : setup-unified-sessions.sh"
  echo "Thời điểm     : $(date '+%Y-%m-%d %H:%M:%S')"
  echo "Máy           : $(whoami) @ $HOME"
  echo "Kit           : $KIT"
  echo "Nội dung      : claude-code-sessions.tgz, claude.json.bak, layout-before.txt, status-before.txt, orig-folders/"
  echo ""
  echo "ROLLBACK (dùng ĐÚNG version đã tạo backup này):"
  echo "  \"$KIT/versions/v$VER/bin/rollback-unified-sessions.sh\" \"$BK\""
} > "$BK/MANIFEST.txt"
note "      ✅ Backup: chỉ mục + claude.json + layout-before + status-before + MANIFEST (v$VER)"
...
note "      ✅ shared: thêm $copied, bỏ qua $skipped ⇒ tổng $({ ls "$SHARED"/local_*.json 2>/dev/null || true; } | wc -l | tr -d ' ') phiên"
